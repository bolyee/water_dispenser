"""
realtime_esp32_mic_denoise.py  —  ESP32 마이크 기반 실시간 수위 모니터 (노이즈 감소 알고리즘 통합 버전)
-------------------------------------------------------------------------------------------------
사용법:
    pip install sounddevice noisereduce scipy
    python realtime_esp32_mic_denoise.py

특징:
    1. 시작 시 ESP32 UDP 스트림(포트 5005)으로부터 2초간 배경 소음(Noise Floor)을 수집하여 필터 생성.
    2. 실시간 및 학습 시 노이즈 제거 필터를 일관되게 적용하여 무선 마이크 환경에서 높은 인식률 확보.
"""
...
# ─────────────────────────────────────────────
#  새 컵 학습 (ESP32 녹음 → 노이즈 제거 → AI 분석 → 캐시 저장)
# ─────────────────────────────────────────────
def calibrate_new_cup(model, noise_profile):
    """ESP32 UDP 마이크를 통해 새 컵 물 따르는 소리를 수집하고 노이즈 필터를 입혀 분석 후 캐시 저장."""
    ...
    print("\n" + "="*58)
    print("  [새 컵 학습 모드 (ESP32 무선 마이크 노이즈 필터)]")
    print("="*58)
    ...
    if noise_profile is not None:
        ...
        print("[경고] 소음 필터가 비활성화되었습니다. 기본 신호 검사만 작동합니다.")
    ...
    if DENOISE_AVAILABLE and noise_profile is not None:
        print("\n⚡ 무선 녹음 데이터에 스펙트럼 차감 잡음 필터를 입히는 중입니다...")
...
def ai_worker(model, l_max, threshold, cache_path, noise_profile):
    """ESP32 UDP 오디오 신호에 노이즈 필터를 실시간 적용하여 템플릿 비교 및 수위를 추정합니다."""
...
def display_loop(l_max, threshold):
    ...
    while True:
        ...
        # Title and Stats
        cv2.putText(canvas, "Real-Time ESP32 Denoised Monitor", (30, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)
        cv2.putText(canvas, "Status: Denoise Filter Active [Bandpass + UDP Spec Subtraction]", (30, 65),
        ...
        cv2.imshow("ESP32 UDP Monitor [Denoised]", canvas)
        try:
...
def main():
    print("=" * 58)
    print("   🎙️  실시간 ESP32 UDP 수위 모니터 [노이즈 제거 버전]   ")
    print("=" * 58)
    ...
    # 2. UDP 리스너 백그라운드 시작 (배경 노이즈 캡처 전 켜져야 함)
    listener_thread = threading.Thread(target=udp_listener, daemon=True)
    listener_thread.start()
    print(f"🎙️  ESP32 UDP 수신 대기 시작 (포트: {UDP_PORT})")
    print("       (ESP32 전원이 켜지고 데이터를 보내고 있어야 합니다.)")
    time.sleep(1.0) # 소켓 연결 및 최초 오디오 버퍼 유입 안정화 대기

    # 3. 배경 소음 측정 (ESP32 UDP 소리 기반)
    noise_profile = None
    if DENOISE_AVAILABLE:
        noise_profile = capture_noise_profile_udp()

    # 4. 컵 선택 (기존 캐시 재사용 or 새 컵 마이크 학습)
