...
class WavelengthWithTime(pl.LightningModule):
    def __init__(
        ...
        if self.use_denoiser:
            ...
            self.denoiser = AudioDenoisingWrapper()
            if denoiser_ckpt and os.path.exists(denoiser_ckpt):
                print(f"[>>>] Loading denoiser weights from {denoiser_ckpt}")
                self.denoiser.unet.load_state_dict(torch.load(denoiser_ckpt, map_location="cpu"))
            else:
                print("[>>>] WARNING: Denoising enabled but no valid checkpoint path provided. Using random weights.")
